...
public class ClientThread extends Thread implements SocketConstants {
	...
	public void oneKeyDeploy(String version) throws Exception {
		...
		this.out.writeObject(SocketData.get(SERVER_ONEKEY_DEPLOY, clientInfo.getType(), version));
	}
	...
	private void _handle(SocketData socketObj) throws Exception {
		...
		case CLIENT_CMD_HEART:
			...
			// DateUtil.getDateYMDHMS());
			this.lastHeartBeatTime = DateUtil.getDateYMDHMS();
			setProjectRuning(Boolean.parseBoolean(socketObj.getMsg_1()));
			setProjectVersion(socketObj.getMsg_2());
			break;
		case CLIENT_REQUIRE_FILE:
			...
			System.out.printf("client[%s] require version[%s] file[%s]\r\n", clientInfo.getClientName(),
					socketObj.getMsg_2(), socketObj.getMsg_3());
	...
	public void setProjectVersion(String projectVersion) {
		this.projectVersion = StringUtils.defaultString(projectVersion, "未知");
	}
